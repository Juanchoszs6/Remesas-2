...
export async function POST(request: Request) {
  ...
  try {
    ...
    const requiredHeaders = ['fecha', 'valor', 'tipo de transacción'];
    const optionalHeaders = ['comprobante', 'factura', 'documento', 'tipo'];
    
    // Primero intentar encontrar los encabezados en las primeras 10 filas
    ...
    // Si no se encontraron encabezados, asumir que los datos empiezan después de las filas de encabezado
    if (headerRowIndex === -1) {
      console.log('No se encontraron encabezados estándar, asumiendo formato personalizado...');
      // Asumir que los datos empiezan después de las primeras 5 filas
      headerRowIndex = 5;
      // Usar encabezados por defecto basados en la posición
      headers = ['tipo', 'fecha', 'documento', 'proveedor', 'descripcion', 'valor'];
      console.log('Usando encabezados por defecto:', headers);
    }
    ...
    // Encontrar índices de columnas con lógica mejorada
    const findColumnIndex = (patterns: string[], required = false): number => {
      for (const pattern of patterns) {
        const index = headers.findIndex(h => 
          h && typeof h === 'string' && h.toLowerCase().includes(pattern)
        );
        if (index !== -1) return index;
      }
      
      // Si es requerido y no se encontró, devolver -1
      if (required) return -1;
      
      // Si no es requerido, intentar adivinar por posición
      if (patterns.some(p => p.includes('fecha') || p.includes('date'))) {
        // Asumir que la fecha está en la segunda columna (índice 1)
        return 1;
      } else if (patterns.some(p => p.includes('valor') || p.includes('monto'))) {
        // Asumir que el valor está en la última columna
    ...
    const fechaIndex = findColumnIndex(['fecha', 'date', 'fech'], true);
    const valorIndex = findColumnIndex(['valor', 'monto', 'total', 'importe', 'val', 'mnt'], true);
    const tipoIndex = findColumnIndex(['tipo', 'type', 'documento', 'doc', 'comp']);
    
    // Validar que las columnas requeridas existan
    if (fechaIndex === -1 || valorIndex === -1) {
      console.log('No se pudieron encontrar las columnas requeridas');
      console.log('Encabezados disponibles:', headers);
      console.log('Primeras filas de datos:', jsonData.slice(0, 5));
      
      return NextResponse.json(
        { 
          success: false,
          error: 'Columnas requeridas no encontradas',
          details: `Se requieren columnas de fecha y valor. Encabezados encontrados: ${headers.join(', ')}`,
          headers: headers,
          sampleData: jsonData.slice(0, 5)
        },
        { status: 400 }
    ...
    for (let i = 0; i < dataRows.length; i++) {
      ...
      // Saltar filas vacías o con muy pocas celdas con datos
      if (!row || row.length < 2 || row.every(cell => cell === null || cell === undefined || cell === '')) {
        console.log(`Fila ${i + 1}: Fila vacía, saltando`);
        skippedRows++;
        continue;
      }
      ...
      try {
        ...
        // Intentar detectar por columna de tipo si existe
        if (tipoIndex !== -1 && row[tipoIndex]) {
          docType = detectDocumentType([row[tipoIndex]], headers);
        }
        
        // Si no se detectó por columna, intentar con toda la fila
        if (!docType) {
          docType = detectDocumentType(row, headers);
        }
        
        // Si aún no se detectó, usar 'FC' como valor por defecto
        if (!docType) {
          console.log(`Fila ${i + 1}: Tipo de documento no detectado, usando FC por defecto`);
          docType = 'FC';
        }
        ...
        if (!date) {
          console.log(`Fila ${i + 1}: Fecha inválida (${row[fechaIndex]}), saltando`);
          skippedRows++;
          continue;
        }
